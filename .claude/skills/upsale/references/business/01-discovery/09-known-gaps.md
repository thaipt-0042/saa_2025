...
- `plans/upsale/scout-report.md` (MUST exist; `[spec]`/`[doc]` bullets supply spec + roadmap paths). **Do NOT Read the full file.** Use `grep -E '\[(spec|doc|other)\]' plans/upsale/scout-report.md` to extract only relevant bullets.
