...
every folder maps to a declared slug; spec.md present OR .pending marker present.
...
def validate(plan_dir: Path) -> dict:
    ...
    for feat in declared:
        ...
        if not spec.is_file() and not pending.is_file():
            issues.append(_issue(
                "critical", "existence.folder_incomplete",
                f"artifacts/features/{slug}",
                "folder has neither spec.md nor .pending marker",
            ))
