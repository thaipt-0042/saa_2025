...
def _make_plan_dir(tmp_path: Path, spec_content: str, fcode: str = "F001_Auth") -> Path:
    """Set up a minimal plan dir with one feature spec."""
    plan_dir = tmp_path / "plan"
    feat_dir = plan_dir / "artifacts" / "features" / fcode
    feat_dir.mkdir(parents=True)
    (feat_dir / "spec.md").write_text(spec_content, encoding="utf-8")
    return plan_dir
...
class TestIncrementalScope:
    ...
    def test_scope_incremental_walks_only_affected(self, tmp_path):
        ...
        f2_content = (plan_dir / "artifacts" / "features" / "F002_Profile" / "spec.md").read_text()
...
class TestMultiBlockAllMissingLinkedFr:
    ...
    def test_three_blocks_all_missing_linked_fr_all_fixed(self, tmp_path):
        ...
        feat_dir.mkdir(parents=True)
        (feat_dir / "spec.md").write_text(spec_src, encoding="utf-8")
        ...
        content = (feat_dir / "spec.md").read_text(encoding="utf-8")
