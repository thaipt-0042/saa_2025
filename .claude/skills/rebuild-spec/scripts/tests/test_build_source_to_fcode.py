...
class TestComputeDocShas:
    def test_computes_sha_for_md_files(self, tmp_path):
        (tmp_path / "route-list.md").write_text("hello")
        (tmp_path / "data-model.md").write_text("world")
    ...
    def test_excludes_non_md(self, tmp_path):
        (tmp_path / "state.json").write_text("{}")
        (tmp_path / "route-list.md").write_text("x")
        shas = _compute_doc_shas(tmp_path)
