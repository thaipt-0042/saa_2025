...
Authoritative spec for the `_canonical-fcodes.json` artifact emitted by Wave 5 alongside `feature-list.md`. Consumed by Wave 5.5 / Wave 6.5 validators and as the slug source for Wave 6 fan-out.
...
| W6 dispatch | TaskCreate prompt prepends `mkdir -p features/{slug}/` (idempotent guard in case W5 skipped) | `.pending` present |
| W6 researcher writes `spec.md` | researcher contract: `rm features/{slug}/.pending` on successful write | `.pending` removed |
| W6 researcher failure | `.pending` remains | `.pending` present (signals partial) |
| W7b reviewer | reads each `features/{slug}/`; if `.pending` present → emit `MISSING` (counts toward `failed`) | — |
...
- Validators (W5.5, W6.5) tolerate absence — when JSON is missing, they emit `[WARN] canonical_missing` and fall back to regex-parsing `feature-list.md` `## Feature Hierarchy` table rows.
...
- `references/pipeline.md` — Wave 5 emit step, Wave 6 fan-out mkdir guard, Wave 5.5 / 6.5 validator gates
